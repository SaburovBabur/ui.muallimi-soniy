...
export async function getLesson({
  ...
  const book = courses().find((course) =>
    course.slug === bookName ? course : false
  );
  ...
  const res = await fetch(
    `${
      process.env.NODE_ENV === "development"
        ? process.env.URL
        : "https://shoshiy-yasiin-uz.vercel.app"
    }/${book.json}`,
    { method: "get" }
  ).then((res) => res.json());
  ...
  return {
    ...
    prev: prev
      ? { title: prev.snippet.title, slug: prev.contentDetails.videoId }
      : null,
    next: next
      ? { title: next.snippet.title, slug: next.contentDetails.videoId }
      : null,
...
function Article({
  ...
  lesson: TypeCourseLesson & { description: string; prev: any; next: any };
}) {
  ...
  if (!lesson) {
    return (
      <HomeLayout>
        <h3 className="h3 flex items-center justify-center pt-10">
